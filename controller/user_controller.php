...
class user_controller extends base_controller
{
	...
	public function details_action()
	{
		...
		{
			$users_stat_ext = $this->users_module->get_user_stats_ext($uid);
			$this->data['u'] = array_merge($this->data['u'], $users_stat_ext);
	...
	{
		$user = $this->users_module->get_by_uid($uid);
	...
	private function get_ban_reason($uid)
	{
		...
		foreach ($rows as $row)
		{
			if ($row['privileges_name'] == 'login')
			{
				...
				{
					$ban_time = " $row[starttime] 至 $row[endtime] ";
				}
