...
class torrents_controller extends base_controller
{
	...
	{
		$privileges_info = $this->check_have_privileges('comment', '发表种子评论');
		if (!$privileges_info['have_privileges'])
		{
			$msg = $privileges_info['msg'];
		...
		$subject = '';
		$message = isset($this->post['message']) ? $this->post['message'] : '';
	...
	private function download()
	{
		...
		if ($this->torrent['uid'] != $this->uid && $this->user['is_user'])
		{
			...
			{
				$privileges_info = $this->check_have_privileges('download_unaudit', '下载未审核通过的种子');
				if (!$privileges_info['have_privileges'])
				{
					$msg = $privileges_info['msg'];
					$this->showmessage($msg, true);
			...
			{
				$privileges_info = $this->check_have_privileges('download_top', '下载置顶推荐免费种子');
				if (!$privileges_info['have_privileges'])
				{
					$msg = $privileges_info['msg'];
					$this->showmessage($msg, true);
			...
			{
				$privileges_info = $this->check_have_privileges('download', '下载种子');
				if (!$privileges_info['have_privileges'])
				{
					$msg = $privileges_info['msg'];
					$this->showmessage($msg, true);
	...
	private function check_seed_count()
	{
		...
		{
			if ($seed_count < $this->setting['hot_torrents_seed_count_limit'])
			{
				$msg = "本种子被设定为热门种子，保种数大于 {$this->setting['hot_torrents_seed_count_limit']} 才能下载。<br />
			您的优特(uTorrent)里面当前种保种数量为 $seed_count , 因此您不能下载本种子，请提高您的保种数量。";
				$this->showmessage($msg);
			}
			if ($seed_size < $this->setting['hot_torrents_seed_size_limit'] * $G)
			{
				$msg = "本种子被设定为热门种子，保种容量大于 {$this->setting['hot_torrents_seed_size_limit']} G 才能下载。<br />
			您的优特(uTorrent)里面当前种保种数量为 $seed_count , 因此您不能下载本种子，请提高您的保种数量。";
				$this->showmessage($msg);
			}
		}
