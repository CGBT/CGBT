...
class cron_controller extends base_controller
{
	...
	private function parse_pic_url($descr)
	{
		$matches = array();
	...
	private function update_bt_info_hash()
	{
	...
	private function get_info_hash($filename)
	{
	...
	private function delete_no_tid_images()
	{
		if (date("H") != '4')
		{
			return;
	...
	private function update_comments_count()
	{
		return;
		cg::load_model('forums_discuzx_model');
	...
	private function update_sitestat()
	{
		cg::load_module('users_module');
		cg::load_module('torrents_module');
	...
	private function delete_no_used_peers_ips()
	{
	...
	private function import_peers_ips()
	{
		cg::load_model('peers_connectable_model');
		$peers_connectable_model = peers_connectable_model::get_instance();
	...
	private function reset_peers_connectable()
	{
	...
	private function check_peers_connectable()
	{
	...
	private function exec($method, $interval)
	{
		$cache_key = 'cron_' . $method;
