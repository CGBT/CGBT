...
class upload_controller extends base_controller
{
	...
	{
		$this->data['privileges_info'] = $this->check_have_privileges('upload', '发布种子');
	...
	private function insert()
	{
		...
		$privileges_info = $this->check_have_privileges('dont_need_audit', '发种无需审核');
	...
	private function get_torrent_info($torrent_file)
	{
		...
		$data = array();
		$data['bt_info_hash'] = sha1(bencode($dict['info']));
		$dict['info']['private'] = 1;
		$dict['info']['source'] = $this->setting['torrents_source'];
		file_put_contents($torrent_file, bencode($dict));

		$data['info_hash'] = sha1(bencode($dict['info']));
		$data['total_length'] = 0;
