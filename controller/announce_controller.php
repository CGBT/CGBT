...
class announce_controller extends cg_controller
{
	...
	private function update_user()
	{
		...
		{
			$upload_factor = $this->get_upload_factor($this->torrent, $this->uid); ////双倍上传在这个函数内配置
			$download_factor = $this->get_download_factor($this->torrent); //全部免费下载在这个函数内配置
			...
			if ($upthis > 0 || $downthis > 0 || $upthis2 > 0 || $downthis2 > 0)
			{
				...
				$set = "uploaded = uploaded + $upthis, uploaded2 = uploaded2 + $upthis2, downloaded = downloaded + $downthis,
				downloaded2 = downloaded2 + $downthis2, today_uploaded = today_uploaded + $upthis, today_uploaded2 = today_uploaded2 + $upthis2,
				today_downloaded = today_downloaded + $downthis, today_downloaded2 = today_downloaded2 + $downthis2,
				last_login = '$this->timestamp', extcredits1 = extcredits1 + $extcredits1";
	...
	private function check_multi_ip_download()
	{
		...
		if ($this->user['privileges']['multi_ip_download'])
		{
			return;
		}
	...
	private function check_agent()
	{
		...
		if (empty($agent))
		{
			$this->failure_message('本站不支持你使用的客户端软件, 推荐使用 uTorrent 3.0 版本 ，主页有下载。');
		}
		...
		foreach ($dict_black_peer_id as $key => $client)
		{
			if (stripos($this->peer_id, $key) !== false)
			{
				...
				{
					$this->failure_message("本站不支持你使用的客户端软件 $client , 推荐使用 uTorrent 3.0 版本 ，主页有下载。");
					break;
				}
			}
		}
		foreach ($this->black_agents as $black_agent)
		{
			...
			{
				$this->failure_message("本站不支持你使用的客户端软件 $agent , 推荐使用 uTorrent 3.0 版本 ，主页有下载。");
				break;
			}
		}
